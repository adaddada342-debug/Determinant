...
game.DescendantAdded:Connect(function(Child)
	-- Guard: Child or Parent may be nil during reparenting
	if not Child then return end
	local parent = Child.Parent
	if not parent then return end

	if Child.Name == "Texture_Storage" and parent:IsA("Configuration") then
		table.insert(TextureDB, parent)

		-- Defensive checks: these may not exist immediately
		local storage = parent:FindFirstChild("Texture_Storage")
		if storage then
			local TS = storage:GetChildren()
			for i = 1, #TS do
				if TS[i].Value ~= nil then
					TS[i].Value:Remove()
					TS[i]:Remove()
		...
		local parsed = parent:FindFirstChild("Texture_Parsed") or parent:WaitForChild("Texture_Parsed", 2)
		if parsed then
			parsed.Value = false
		end
...
game.DescendantRemoving:Connect(function(Child)
	-- Guard: Child or Parent may be nil during destruction/reparenting
	if not Child then return end
	local parent = Child.Parent
	if not parent then return end

	if Child.Name == "Texture_Storage" and parent:IsA("Configuration") then
		local Identify = table.find(TextureDB, parent)
		if Identify then
	...
	for i = 1,#TextureDB do
		if TextureDB[i].Parent:IsA("BasePart") then
			if TextureDB[i].Texture_Optical.Value == true then
				...
				local ListSize2 = {TextureDB[i].Parent.Size.X,TextureDB[i].Parent.Size.Y,TextureDB[i].Parent.Size.Z}
				table.sort(ListSize2, function(a, b)
					return a > b
				end)
				...
				-- FIX: FindFirstAncestor expects a string. Use IsDescendantOf instead.
				local char = localPlayer and localPlayer.Character
				local PLRAC = (char ~= nil) and TextureDB[i]:IsDescendantOf(char)

				if WPS == true or localPlayer:GetMouse().Target == TextureDB[i].Parent or PLRAC then
					if (TextureDB[i].Parent.Position-game.Workspace.CurrentCamera.CFrame.Position).Magnitude < 100+ListSize[1] then
						...
						for j = 1,#TXT3 do
							if TXT3[j].Value ~= nil and TXDB ~= nil then
								TXT3[j].Value.OffsetStudsV += game:GetService("UserInputService"):GetMouseDelta().Y*TXDB:GetAttribute("SeedA")*TXDB.Texture_Optical.Optical_Shift.Value
								TXT3[j].Value.OffsetStudsU += game:GetService("UserInputService"):GetMouseDelta().X*TXDB:GetAttribute("SeedB")*TXDB.Texture_Optical.Optical_Shift.Value
								TXT3[j].Value.StudsPerTileU = TOB.Texture_Size.Value
								TXT3[j].Value.StudsPerTileV = TOB.Texture_Size.Value
							end
						end
					end
				else
			...
			if TextureDB[i].Texture_Moving.Value == true then
				...
				if WPS == true or localPlayer:GetMouse().Target == TextureDB[i].Parent or PLRAC then
					if (TextureDB[i].Parent.Position-game.Workspace.CurrentCamera.CFrame.Position).Magnitude < 100+ListSize[1] then
						local TXT3 = TextureDB[i].Texture_Storage:GetChildren()
						for j = 1,#TXT3 do
							if TXT3[j].Value ~= nil then
								TXT3[j].Value.OffsetStudsV += TOB.Texture_Moving.SpeedB.Value
								TXT3[j].Value.OffsetStudsU += TOB.Texture_Moving.Speed.Value
								TXT3[j].Value.StudsPerTileU = TOB.Texture_Size.Value
								TXT3[j].Value.StudsPerTileV = TOB.Texture_Size.Value
								if TXT3[j].Value.OffsetStudsV > 10000 or TXT3[j].Value.OffsetStudsU > 10000 then
									TXT3[j].Value.OffsetStudsV = 0
									TXT3[j].Value.OffsetStudsU = 0
								end
							end
						end
					end
